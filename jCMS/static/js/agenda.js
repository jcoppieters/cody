function initAgenda() {
	...
	$("#bar #q").keyup( function (event) { 
		if (event.which == 13) { 
			document.location.href = gContext + "/nl/agenda?request=search&q=" + $(this).val() 
		} 
	});
	...
	if ($("#patDialog").length > 0) {
		...
		$("#patDialog #patName")
		  .autocomplete({ 
			source: gContext + "/nl/patients/search",
...
function changeDate() { 
	document.location.href = gContext + "/nl/agenda?date=" + $("#currDate").val(); 
}
